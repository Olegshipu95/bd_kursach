CREATE OR REPLACE FUNCTION check_instrument_status()
    RETURNS TRIGGER AS $$
BEGIN
    IF NEW.instrument_id IS NOT NULL THEN
        -- Проверка статуса в таблице instrument
        IF (SELECT status FROM instrument WHERE id = NEW.instrument_id) = 1 THEN
            RAISE EXCEPTION 'Instrument with status 1 cannot be used for sacrifice';
        END IF;
    END IF;
    RETURN NEW;
END;
...
CREATE TRIGGER before_insert_sacrifice
    ...
    ON sacrifice
    FOR EACH ROW
EXECUTE FUNCTION check_instrument_status();
